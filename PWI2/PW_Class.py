...
"""
"""
This script is to communicate with PWI2, by PlaneWave Instruemnts using the python functions below.
...
class PWI2():   
    def __init__(self):
        """
        This function initializes the connection to PWI2. 
        It also defines names for the variables from PWI2, which are stored in self. 
        
        Returns: True
        """
        #this is the IP address of PWI2
        self.link="http://192.168.71.40:8080/"
        #Here I request the data with the link + a command. This seems like a good idea?
        self.data = requests.get(self.link+"?cmd=getsystem")
        #Creates an XML tree of the data from the request
        self.tree = ET.fromstring(self.data.content)

        #tree[x][y] where x is the child of system and y is the grandchild (naming structure??)
        #So print(tree[4][1].text) prints out the value from the 5th child and 2nd grandchild.
        #Updates the values in the data by making a new request to the server.
    ...
    def getUTC(self):
        """
        Prints the current UTC time in the format HH:MM:SS
    ...
    def getJD(self):
        ...
        """
        Prints the current Julian Date
    ...
    def getFocuserPos(self):
        ...
        if self.Foc_connection == "True":
            ...
            print('The focuser is not connected to PWI2. Please use .ConnectFOC() to connect the focuser to PWI2.')
            reply = 'The focuser is not connected to PWI2. Please use .ConnectFOC() to connect the focuser to PWI2.'
            return reply
    ...
    def getRotatorPos(self):
        ...
        """
        self.update()
        print('The current position of the rotator is %s degrees'%(self.ROT_position))
        reply = 'The current position of the rotator is %s degrees'%(self.ROT_position)
        return reply
    ...
    def getStatus(self):
        ...
        Alt = self.MNT_Alt
        Azm = self.MNT_Azm
        On_target = self.MNT_on_target #From the documentation, PWI recommends that I use on_target instead of moving!
        ...
        reply = "The telescope is pointed at RA: %s, DEC:%s (J2000) / Alt: %.4f, Azm: %.4f and the time is %s UTC. \n" %(RA,DEC,float(Alt)*180/np.pi,float(Azm)*180/np.pi,UTC)
        
        
        print('The telescope is pointed at RA: %s, DEC:%s (J2000) / Alt: %.4f, Azm: %.4f and the time is %s UTC.' %(RA,DEC,float(Alt)*180/np.pi,float(Azm)*180/np.pi,UTC))
        if On_target == "False":
        ...
        if Rot == "True":
            ...
            print('You should maybe try to reconnect the focuser and theirby reconnect the rotator with the functions: .DisconnectFOC() and .ConnectFOC()\n')
    ...
    def getMNT_CONNECT(self):
        ...
        """
        self.update()
        MNT_CONNECT = self.MNT_connection

        if MNT_CONNECT == "True":
            print("Mount is connected")
            return MNT_CONNECT #MNT_CONNECT
        else:
    ...
    def getFOC_CONNECT(self):
        ...
        self.update()
        Focuser_CONNECT = self.Foc_connection

        if Focuser_CONNECT == "True":
            print("Focuser is connected")
            return Focuser_CONNECT
        else:
    ...
    def getROT_CONNECT(self):
        ...
        self.update()
        Rotator_CONNECT = self.ROT_connection

        if Rotator_CONNECT == "True":
            print("Rotator is connected")
            return Rotator_CONNECT
        else:
    ...
    def getTrackingRMSError(self):
        ...
        self.update()
        print('The RMS error on the Alt pointing is %s arcseconds'%(self.MNT_AltRMSErrorArcsec))
        
        print('The RMS error on the Azm pointing is %s arcseconds'%(self.MNT_AzmRMSErrorArcsec))
        reply = 'The RMS error on the Alt pointing is %s arcseconds \n'%(self.MNT_AltRMSErrorArcsec)
        reply += 'The RMS error on the Azm pointing is %s arcseconds'%(self.MNT_AzmRMSErrorArcsec)
        return reply
    ...
    def MoveFocuserPos(self,position):
        ...
        cmd=requests.get(self.link+"?&device=focuser&cmd=move&position=%i"%(position))
        time.sleep(3)
        self.update()
        ...
        while self.Foc_move == "True" and self.Foc_complete == 'False':
    ...
    def ConnectFOC(self):
        ...
        since they share the same connection to PWI
        
        Returns:
            True if connection is complete.
            False if timeout occurs.
        """
        FOC = requests.get(self.link+"?&device=focuser&cmd=connect")
    ...
    def DisconnectFOC(self):
        ...
        Returns:
            ...
            False for timeout
        """
        FOC = requests.get(self.link+"?&device=focuser&cmd=disconnect")
        self.update()
        timeout_sec = 20
        timeout = time.time() + timeout_sec
    ...
    def MoveFocuserInc(self,increment):
        ...
        The increments add i.e. moving the focuser 100 microns and then 
        ...
        if abs(self.Foc_pos - self.FocInc_target) < 5: #checks that the new position is within 5 micron of the target. 
            ...
            print('The focuser did not reach the target position. The current position is %s microns'%(self.Foc_pos))
            return False
    ...
    def FocSTOP(self):
        ...
        cmd = requests.get(self.link+"?&device=focuser&cmd=stop")
        timeout_sec = 20 #number of seconds before timeout
        timeout = time.time() + timeout_sec
        time.sleep(3)
        self.update()
        while self.Foc_move == "True":
    ...
    def FocFindHome(self):
        """
        A function that homes the focuser. The homing-procedure moves the focuser to 0 microns, 
        ...
        time.sleep(2)
        self.update()
        while self.Foc_homing == "True" and self.Foc_move == "True":
    ...
    def ConnectMNT(self):
        ...
        if self.MNT_connection == "True" and self.MNT_AzmMotor == "True" and self.MNT_AltMotor == "True":
            ...
            return True
        cmd = requests.get(self.link+"?&device=mount&cmd=connect")
        self.update()
        time.sleep(1)
        ...
        time.sleep(1)
        enable = requests.get(self.link+"?&device=mount&cmd=enable")
        self.update()
    ...
    def DisconnectMNT(self):
        ...
        if self.MNT_connection == "False":
            ...
            return True
        disable = requests.get(self.link+"?&device=mount&cmd=disable")
        time.sleep(2)
        cmd = requests.get(self.link+"?&device=mount&cmd=disconnect")
        self.update()
        time.sleep(2)
        ...
        if check == 0:
            ...
            if (str(star_alt)[1:2]) == ':': #The return value of the altitude of the star, when the Alt is below 10, is i.e. 8 and not 08. The code crashed beforehand due to "8:" not being an integer
                if int(str(star_alt)[0:1]) > horizon_limit:
                    ...
                    track = requests.get(self.link+"&?device=mount&cmd=move&ra2000=%s&dec2000=%s"%(RA.replace(':',' '),DEC.replace(':', ' ')))
    ...
    def stopTracking(self):
        ...
        stop = requests.get(self.link+"?&device=mount&cmd=trackingoff")
        ...
        if self.MNT_tracking == "False":
            ...
            time.sleep(3)
            self.update()
            print('The current position of the telescope is %g\xb0 %g\xb0 (Alt,Azm)'%(float(self.MNT_Alt)*180/np.pi,float(self.MNT_Azm)*180/np.pi)) #convert radians to deg
        else: 
    ...
    def parkMount(self):
        ...
        park = requests.get(self.link+"?&device=mount&cmd=move&alt=%i&azm=%i"%(park_alt,park_azm))
    ...
    def MountSTOP(self):
        ...
        cmd = requests.get(self.link+'?&device=mount&cmd=stop')
        timeout_sec = 15 #Number of seconds before timeout
        timeout = time.time()+timeout_sec
        while self.MNT_moving == "False" and self.MNT_tracking == "False":
    ...
    def MntMotorEnable(self):
        ...
        cmd = requests.get(self.link+"?&device=mount&cmd=enable")
        time.sleep(1)
        self.update()
    ...
    def MntMotorDisable(self):
        ...
        cmd = requests.get(self.link+"?&device=mount&cmd=disable")
        time.sleep(1)
        self.update()
    ...
    def MntMoveRaDec(self):
        ...
        if int(alt_degree) > horizon_limit:
            ...
            cmd = requests.get(self.link+"?&device=mount&cmd=move&ra=%s&dec=%s"%(RA.replace(":"," "),DEC.replace(":"," ")))
...
#            return False
        ...
        print('Starting position is at %.4f\xb0 %.4f\xb0 (Alt/Azm)'%(float(self.MNT_Alt)*180/np.pi,float(self.MNT_Azm)*180/np.pi))
        cmd = requests.get(self.link+"?&device=mount&cmd=move&alt=%i&azm=%i"%(Alt,Azm))
        timeout_sec = 60 #number of seconds before timing out.
        timeout = time.time() + timeout_sec
        time.sleep(2)
        self.update()
        while self.MNT_on_target == "False":
            ...
            if time.time() > timeout:
                ...
                print('The position at timeout was Alt: %s and Azm: %s'%(float(self.MNT_Alt)*180/np.pi,float(self.MNT_Azm)*180/np.pi))
                return False
        print('The mount is on target and is tracking at %.3f\xb0,%.3f\xb0 (Alt/Azm)'%(float(self.MNT_Alt)*180/np.pi,float(self.MNT_Azm)*180/np.pi))
        return True
    ...
    def startTracking(self):
        ...
        cmd = requests.get(self.link+"?&device=mount&cmd=trackingon")
    ...
    def startMntHoming(self):
        ...
        *PLEASE NOTE*: Per version 0.0.1 this function crashes the simulator and
        ...
        cmd = requests.get(self.link+'?&device=mount&cmd=findhome')
        return
    ...
    def SyncMountCoorJ2000(self,RA,DEC):
        ...
        #Adding a point/target to the current pointing model. It assigns the current telescope position to the provided J2000 RA and DEC.
        #I check the format of RA/DEC is correct before parsing it further. 
    def AddPointToModel(self,RA,DEC):
        ...
        cmd = requests.get(self.link+"?&device=mount&cmd=addtomodel&ra2000=%s&dec2000=%s"%(RA.replace(":"," "),DEC.replace(":"," ")))
        ...
        #A function that saves the current pointing model in Documents/PlaneWave Instruments/PWI2/Mount
        #I check that the filename is alphanumeric (but can still contain hyphen and underscore)
        # 
    ...
    def RotSTOP(self):
        ...
        cmd = requests.get(self.link+"?&device=rotator&cmd=stop")
        time.sleep(1)
        self.update()
        timeout_sec = 15 #number of seconds before timeout
        timeout = time.time() + timeout_sec
